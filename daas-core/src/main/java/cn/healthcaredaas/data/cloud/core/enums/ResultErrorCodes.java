 ...
 * 其它内容逐步补充
 *
 * @ClassName： ResultErrorCodes.java
 * @Author： chenpan
 * @Date：2024/11/28 15:27
 * @Modify：
 */
@Schema(title = "响应结果状态", description = "自定义错误码以及对应的错误信息")
public enum ResultErrorCodes {

    /**
     * 401.** 未经授权 Unauthorized	请求要求用户的身份认证
     */
    INSUFFICIENT_AUTHORIZED(ErrorCode.UNAUTHORIZED, "缺少认证信息"),
    ...
    INVALID_CLIENT(ErrorCode.INVALID_CLIENT, "客户端ID或密码错误"),
    ...
    USERNAME_NOT_FOUND(ErrorCode.USERNAME_NOT_FOUND, "用户名不存在"),
    ...
    HTTP_MESSAGE_NOT_READABLE_EXCEPTION(ErrorCode.HTTP_MESSAGE_NOT_READABLE_EXCEPTION, "请求参数格式错误！"),
    ...
    NULL_POINTER_EXCEPTION(ErrorCode.NULL_POINTER_EXCEPTION, "系统错误"),
    ...
    BAD_SQL_GRAMMAR(ErrorCode.BAD_SQL_GRAMMAR, "SQL语法错误，检查SQL能否正常运行或者字段名称是否正确"),
    DATA_INTEGRITY_VIOLATION(ErrorCode.DATA_INTEGRITY_VIOLATION, "数据完整性约束异常，请检查数据内容"),
